...
const fetchInstructorProfile = async (instructorId: string) => {
  ...
  const res = await axios.get(`http://192.168.1.139:3000/api/user/${instructorId}`);
  return res.data;
};

const fetchSessions = async () => {
  const token = await SecureStore.getItemAsync('token');
  const res = await axios.get('http://192.168.1.139:3000/api/attendance/instructor-sessions', {
